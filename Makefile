...
bundle: manifests operator-sdk kustomize yq ## Generate OLM bundle manifests and metadata, then validate
	...
	# relatedImages is generated by operator-sdk above, cannot be set via kustomize; update agent image relatedImages here
	$(YQ) --yaml-compact-seq-indent eval '(.spec.relatedImages[] | select(.name == "sbr-agent")).image |= "$(AGENT_IMG)"' -i bundle/manifests/storage-based-remediation.clusterserviceversion.yaml
	# RELATED_IMAGE_SBR_AGENT key is "value", not "image", cannot be set via kustomize; update agent image env var here
	$(YQ) --yaml-compact-seq-indent eval '(.spec.install.spec.deployments[0].spec.template.spec.containers[0].env[] | select(.name == "RELATED_IMAGE_SBR_AGENT")).value |= "$(AGENT_IMG)"' -i bundle/manifests/storage-based-remediation.clusterserviceversion.yaml
